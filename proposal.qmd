...
-   Correlation heatmaps to examine inter-trait relationships or correlation matrix
...
-   I will use encode target variable (animals taxa) using `LabelEncoder`. For the evolutionary rates, I may convert any values \> 0 to 1 for the presence of sexually selected traits and hot-encode these numerical columns (sexually selected traits).

-   For `evolution_df`, I will standardize feature values using `StandardScaler`.
...
-   I may do a classification report on which family has sexually selected traits and explain the results (if I have time for the presentation).